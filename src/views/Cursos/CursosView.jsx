...
import { 
  ...
  GraduationCap, Users, MapPin, Clock, DollarSign
} from "lucide-react";
...
export default function CursosView({ tipo = "CURSOS" }) {
  const { cursos, loading, institucion, portadas } = useOutletContext(); // ← Agregamos portadas
                  ...
                  >
                    ...
                    <div className="relative h-48 overflow-hidden bg-black/20">
                      {item.det_img_portada && item.det_img_portada.startsWith('http') ? (
                        ...
                        >
                          <typeStyle.icon size={48} style={{ color: primaryColor }} className="opacity-40" />
                        </div>
                      )}
                      ...
                      {/* Badge modalidad */}
                      <span className="absolute top-3 right-3 bg-black/60 backdrop-blur-sm text-white text-[10px] font-bold px-2 py-1 rounded-full shadow-md border border-white/10">
                        {item.det_modalidad}
                      </span>

                      {/* Badge estado */}
                      {isActive && (
                        <span className="absolute bottom-3 left-3 bg-green-500 text-white text-[10px] font-bold px-2 py-1 rounded-full shadow-lg border border-white/20">
                          EN CURSO
                        </span>
                      )}
                      {isExpired && (
                        <span className="absolute bottom-3 left-3 bg-red-500 text-white text-[10px] font-bold px-2 py-1 rounded-full shadow-lg border border-white/20">
                          FINALIZADO
                        </span>
                      )}
                    </div>
