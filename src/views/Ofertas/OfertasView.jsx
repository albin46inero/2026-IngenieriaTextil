    ...
    return (
      ...
      >
        <div className="relative z-10 max-w-4xl">
          <Briefcase size={80} style={{ color: primaryColor }} className="mx-auto mb-6 opacity-60" />
...
export default function OfertasView() {
  const { ofertas, loading, institucion, portadas } = useOutletContext(); // ← Agregamos portadas
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-16">
        ...
        {filteredItems.length === 0 ? (
          ...
          >
            ...
            <p className="text-white/60">No hay ofertas académicas disponibles</p>
          </motion.div>
        ) : (
                  ...
                  >
                    ...
                    <div className="relative h-48 overflow-hidden bg-black/20">
                      {item.ofertas_imagen && item.ofertas_imagen.startsWith('http') ? (
                        ...
                        >
                          <Briefcase size={48} style={{ color: primaryColor }} className="opacity-40" />
                        </div>
                      )}
                      
                      {/* Badge estado */}
                      {isActive && (
                        <span className="absolute top-3 right-3 bg-green-500 text-white text-[10px] font-bold px-2 py-1 rounded-full shadow-lg border border-white/20">
                          INSCRIPCIONES ABIERTAS
                        </span>
                      )}
                      {isExpired && (
                        <span className="absolute top-3 right-3 bg-red-500 text-white text-[10px] font-bold px-2 py-1 rounded-full shadow-lg border border-white/20">
                          FINALIZADO
                        </span>
                      )}
                    </div>
